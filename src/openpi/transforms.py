...
class RepackTransform(DataTransformFn):
    ...
    A leaf can also be a sequence of candidate paths. In that case the first existing path
    will be used and the leaf will be dropped if none of the candidates exist.
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        repacked = _resolve_repack_structure(self.structure, flat_item)
        if repacked is _MISSING_REPACK_VALUE:
            return {}
        return repacked
...
class ResizeImages(DataTransformFn):
    ...
    def __call__(self, data: DataDict) -> DataDict:
        if self.key in data:
            data[self.key] = {
                k: image_tools.resize_with_pad(self.to_numpy(v), self.height, self.width)
...
_MISSING_REPACK_VALUE = object()


def _resolve_repack_structure(structure: at.PyTree, flat_item: dict[str, DataDict]):
    if isinstance(structure, Mapping):
        out = {}
        for key, value in structure.items():
            resolved = _resolve_repack_structure(value, flat_item)
            if resolved is not _MISSING_REPACK_VALUE:
                out[key] = resolved
        return out if out else _MISSING_REPACK_VALUE

    if isinstance(structure, str):
        return flat_item[structure]

    if isinstance(structure, Sequence) and not isinstance(structure, (str, bytes)):
        for candidate in structure:
            if candidate in flat_item:
                return flat_item[candidate]
        return _MISSING_REPACK_VALUE

    raise TypeError(f"Unsupported repack leaf type: {type(structure)!r}")
