...
class DepthPatchTokenizer(nnx.Module):
    def __init__(self, patch_size: int, token_dim: int, *, rngs: nnx.Rngs, param_dtype=jnp.float32):
        ...
        self.camera_embeddings = [
            nnx.Param(jax.random.normal(rngs.params(), (1, 1, token_dim), dtype=jnp.float32) * 0.02)
            for _ in _model.DEPTH_IMAGE_KEYS
        ]

    def __call__(
        self,
        depth_images: dict[str, jax.Array],
        depth_image_masks: dict[str, jax.Array] | None = None,
    ) -> jax.Array | None:
        depth_tokens = []
        for camera_idx, camera_name in enumerate(_model.DEPTH_IMAGE_KEYS):
            if camera_name not in depth_images:
                continue

            depth_image = depth_images[camera_name]
            if depth_image.shape[-1] != 1:
                depth_image = depth_image[..., :1]

            height, width = depth_image.shape[1:3]
            if height % self.patch_size != 0 or width % self.patch_size != 0:
                raise ValueError(
                    f"Depth image {camera_name} shape {(height, width)} is not divisible by patch size {self.patch_size}."
                )
            ...
            patch_tokens = patch_tokens + self.camera_embeddings[camera_idx]

            if depth_image_masks is not None and camera_name in depth_image_masks:
                patch_tokens = patch_tokens * depth_image_masks[camera_name][:, None, None].astype(patch_tokens.dtype)

            depth_tokens.append(patch_tokens)

        if not depth_tokens:
            return None

        return jnp.concatenate(depth_tokens, axis=1)
...
class ACOTConfig(_model.BaseModelConfig):
    ...
    depth_patch_size: int = 32
    depth_token_dim: int = 256
    depth_in_ear: bool = True
    depth_in_agp: bool = True
    ...
    def inputs_spec(self, *, batch_size: int = 1) -> tuple[_model.Observation, _model.Actions]:
        ...
        with at.disable_typechecking():
            observation_spec = _model.Observation(
                ...
                depth_images={
                    "base_0_depth": jax.ShapeDtypeStruct([batch_size, *_model.IMAGE_RESOLUTION, 1], jnp.float32),
                    "left_wrist_0_depth": jax.ShapeDtypeStruct([batch_size, *_model.IMAGE_RESOLUTION, 1], jnp.float32),
                    "right_wrist_0_depth": jax.ShapeDtypeStruct([batch_size, *_model.IMAGE_RESOLUTION, 1], jnp.float32),
                }
                if self.use_depth
                else {},
                depth_image_masks={
                    "base_0_depth": image_mask_spec,
                    "left_wrist_0_depth": image_mask_spec,
                    "right_wrist_0_depth": image_mask_spec,
                }
                if self.use_depth
                else {},
            )
...
class ACOT_VLA(_model.BaseModel):
    def __init__(self, config: ACOTConfig, rngs: nnx.Rngs):
        ...
        self.depth_in_ear = config.use_depth and config.depth_in_ear
        self.depth_in_agp = config.use_depth and config.depth_in_agp

        if self.use_depth:
            self.depth_tokenizer = DepthPatchTokenizer(
                patch_size=config.depth_patch_size,
                token_dim=config.depth_token_dim,
                rngs=rngs,
            )

            if self.depth_in_ear:
                self.ear_depth_adapter = UnifiedAttentionModule(
                    in_dim_1=coarse_action_expert_config.width,
                    in_dim_2=config.depth_token_dim,
                    out_dim=coarse_action_expert_config.width,
                    hidden_dim=coarse_action_expert_config.width,
                    apply_sigmoid=False,
                    num_heads=4,
                    rngs=rngs,
                )

            if self.depth_in_agp:
                self.agp_depth_adapter = UnifiedAttentionModule(
    ...
    def embed_suffix(
        ...
        if suf_type == "reasoner":
            ...
            if self.depth_in_ear and depth_tokens is not None:
                action_expert_tokens = action_expert_tokens + self.ear_depth_adapter(action_expert_tokens, depth_tokens)
            ...
            if self.depth_in_agp and depth_tokens is not None:
                action_expert_tokens = action_expert_tokens + self.agp_depth_adapter(action_expert_tokens, depth_tokens)
    ...
    def encode_depth_tokens(self, obs: _model.Observation) -> jax.Array | None:
        if not self.use_depth or not obs.depth_images:
            return None
        return self.depth_tokenizer(obs.depth_images, obs.depth_image_masks)
    ...
    def compute_loss(
        ...
        depth_tokens = self.encode_depth_tokens(observation)
